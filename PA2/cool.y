...
%{
  ...
  for the location of tokens */
    ...
    */
    ...
    %type <expressions> exp_parameter
    %type <expression> exp
    ...
    */
    ...
    | OBJECTID ':' TYPEID ASSIGN exp { $$ = attr($1, $3, $5); }
    ;
    ...
    exp_parameter
    : exp { $$ = single_Expressions($1); }
    | exp_parameter ',' exp { $$ = append_Expressions($1, single_Expressions($3)); }
    | { $$ = nil_Expressions(); }
    | error ',' { ; }
    ;

    exp_let
    : OBJECTID ':' TYPEID IN exp { $$ = let($1, $3, no_expr(), $5); }
    | OBJECTID ':' TYPEID ASSIGN exp IN exp { $$ = let($1, $3, $5, $7); }
    | OBJECTID ':' TYPEID ',' exp_let { $$ = let($1, $3, no_expr(), $5); }
    | OBJECTID ':' TYPEID ASSIGN exp ',' exp_let { $$ = let($1, $3, $5, $7); }
    | error exp_let { ; }
    ;

    exp
    : OBJECTID ASSIGN exp { $$ = assign($1, $3); }
    | exp '@' TYPEID '.' OBJECTID '(' exp_parameter ')' { $$ = static_dispatch($1, $3, $5, $7); }
    | OBJECTID '(' exp_parameter ')' { $$ = dispatch(object(idtable.add_string("self")), $1, $3); }
    | exp '.' OBJECTID '(' exp_parameter ')' { $$ = dispatch($1, $3, $5); }
    | IF exp THEN exp ELSE exp FI { $$ = cond($2, $4, $6); }
    | WHILE exp LOOP exp POOL { $$ = loop($2, $4); }
    | '{' exp_list '}' { $$ = block($2); }
    | LET exp_let { $$ = $2; }
    | CASE exp OF case_list ESAC { $$ = typcase($2, $4); }
    | NEW TYPEID { $$ = new_($2); }
    | ISVOID exp { $$ = isvoid($2); }
    | exp '+' exp { $$ = plus($1, $3); }
    | exp '-' exp { $$ = sub($1, $3); }
